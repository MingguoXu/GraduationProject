...
/*---------------------------------------------------------------------------*/
#define MOSQ_MSB(A)         (u8)((A & 0xFF00) >> 8)
#define MOSQ_LSB(A)         (u8)(A & 0x00FF)
...
{
    char*  _data;          /* buffer数据 */
    u32  _write_pos;     /* buffer写入位置 */
    u32  _read_pos;      /* buffer读取位置 */
    u32  _capacity;      /* buffer容量 */
 ...
 */
int UnpackPushdata(EdpPacket* pkg, char** src_devid, 
